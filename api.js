...
io.on('connection', function (socket) {
  ...
  socket.on('disconnect', function () {
    ...
    room_list.forEach((room, index) => {
      leaving_player = room.player_list.find(
        (x) => x.id == socket.id && x.is_online
      );
      ...
      if (leaving_player != undefined) {
        if (!leaving_player.spectator) {
          ...
          room.player_list = room.player_list.filter(
            (x) => x.name != leaving_player.name
          );
  ...
  socket.on('kick', function (room_name, kicked_player) {
    ...
    if (player != undefined) {
      if (
        ...
        //check se kickato un giocatore in gioco (stupido admin)
        annulla_game = true;
      }
      room.player_list = room.player_list.filter(
        (x) => x.name != kicked_player
      );

      if (room.player_list.length <= 2) {
        //check se giocatori rimasti >= 3
        annulla_game = true;
      }
  ...
  socket.on('vote', function (data, callback) {
    ...
    if (
      room.player_list.filter((x) => x.role.detail != '' && !x.spectator)
        .length == room.player_list.filter((x) => x.has_voted).length
    ) {
      room.game_started = false;
    ...
    if (room.game_started == false && room.vote_ended == true) {
      ...
      room.player_list
        .filter((x) => x.role.detail != '' && !x.spectator)
        .forEach((player) => {
          console.log(player.name);
          if (
            player.role.name != 'Assassino' &&
            player.role.name != 'Mitomane'
          ) {
            //Calcolo per buoni
            if (player.player_voted == 'Cielo') {
              //Se voto al cielo controllo per assassini in gioco
              if (
                room.player_list.filter((x) => x.role.name == 'Assassino')
                  .length == 0
              ) {
                bonus = true;
              } else {
              ...
              //Se il voto non è al cielo controllo il ruolo del giocatore votato
            ...
            //Se sei un cattivo conto in quanti ti hanno votato e in quanti invece no
            if (player.role.name == 'Assassino' && calcolo_assassini) {
              //calcolo_assassini = booleano per fare il calcolo una sola volta e non ogni volta per ogni assassino!
              calcolo_assassini = false;
              room.player_list
                .filter(
                  (x) =>
                    x.role.name != 'Assassino' &&
                    x.role.name != 'Mitomane' &&
                    !x.spectator
                )
                .forEach((pp) => {
                  if (pp.player_voted != 'Cielo') {
                    var player_voted = room.player_list.find(
                      (x) => x.name == pp.player_voted
                    );
            ...
            if (player.role.name == 'Mitomane') {
              tot_voti_mitomane = room.player_list
                .filter(
                  (x) =>
                    x.role.name != 'Assassino' &&
                    x.role.name != 'Mitomane' &&
                ...
                .filter((y) => y.player_voted == player.name).length;
              tot_non_voti_mitomane = room.player_list
                .filter(
                  (x) =>
                    x.role.name != 'Assassino' &&
                    x.role.name != 'Mitomane' &&
  ...
  socket.on('game', function (data, callback) {
    ...
    var players_online = room.player_list
      .filter((x) => x.is_online && !x.spectator)
      .slice();
    ...
    if (tot_players == 2) {
      //Se giocatori sono due aggiungi solo o un lupo o un mitomane random
    ...
    for (var player of players_online) {
      var random_item = Math.floor(
        Math.random() * configurazione_attiva.length
      );
    ...
    var assassini = players_online
      .filter((x) => x.role.name == 'Assassino')
      .slice();
    ...
    players_online.forEach((player) => {
      ...
      switch (player.role.name) {
        ...
        case 'Cittadina':
          player.role.detail =
            'Sei un semplice cittadino in una città piena di assassini... guardati le spalle';
          break;
        case 'Investigatore':
          player.role.detail =
            'Hai veggiato nel mezzo: ' +
            esiliate[Math.floor(Math.random() * esiliate.length)];
          break;
        case 'Investigatrice':
          player.role.detail =
            'Hai veggiato nel mezzo: ' +
        ...
        case 'Testimone':
          ...
          if (altro_testimone == '') {
            ...
            if (room.testimoni_si_riconoscono) {
              player.role.detail =
                'Sei testimone con ' + altro_testimone.slice(0, -3);
            } else {
              if (tottestimoni > 1) {
                player.role.detail =
                  'Sei testimone insieme ad altri ' +
                  tottestimoni +
                  ' giocatori!';
              } else {
          ...
          break;
        case 'Avvocato':
          console.log('ciao avvocato!');
          for (var t of players_online) {
            if (
              t.role.name != 'Assassino' &&
              t.role.name != 'Mitomane' &&
              t.name != player.name
            ) {
              player.role.detail =
...
break;
        ...
        case 'Mitomane':
          if (room.mitomane_riconosce_assassini) {
            if (assassini.length == 3)
              player.role.detail =
                'Sei il mitomane e gli assassini sono ' +
                assassini[0].name +
                ', ' +
                assassini[1].name +
                ' e ' +
                assassini[2].name;
            if (assassini.length == 2)
              player.role.detail =
                'Sei il mitomane e gli assassini sono ' +
                assassini[0].name +
                ' e ' +
                assassini[1].name;
            if (assassini.length == 1)
              player.role.detail =
                "Sei il mitomane e l'assassino è " + assassini[0].name;
            if (assassini.length == 0)
              player.role.detail =
                'Sei il mitomane e non ci sono assassini in giro ad aiutarti';
          } else {
        ...
        case 'Assassino':
          ...
          if (altro_assassino == '') {
            ...
            player.role.detail =
              'Sei assassino con ' + altro_assassino.slice(0, -3);
          }
          break;
      }
      io.to(`${player.id}`).emit('role', player);
